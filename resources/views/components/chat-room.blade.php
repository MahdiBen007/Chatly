<div
    ...
    <!-- Header -->
    <div class="flex items-center justify-between border-b border-gray-300 dark:border-gray-700 bg-white dark:bg-transparent px-4 py-3"
        data-room-key="{{ $roomKey }}">
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-full bg-blue-100 dark:bg-primary/20 flex items-center justify-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 text-blue-500 dark:text-primary" fill="none"
            ...
            <div>
                <h3 class="font-semibold text-gray-900 dark:text-gray-100">{{ $roomName }}</h3>
    ...
    <div id="chat-messages"
        class="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar bg-gray-50 dark:bg-transparent"
        data-room-key="{{ $roomKey }}">
            ...
            @if ($message->sender_id === auth()->id())
                <div class="flex items-end gap-2 justify-end" data-message-id="{{ $message->id }}">
                    <div>
                            ...
                            @if ($message->file_path)
                                ...
                                @if ($isImage)
                                    <img src="{{ $fileUrl }}" alt="Attached image"
                                        class="max-w-full h-auto rounded-lg mb-2" style="max-height: 300px;">
                                @else
                                    <a href="{{ $fileUrl }}" download
                                        class="flex items-center gap-2 text-white hover:underline mb-2">
                        ...
                        <span class="text-xs text-gray-400 flex justify-end mr-1">{{ $message->created_at->format('h:i') }}</span>
                    </div>
                </div>
            @else
                <div class="flex items-end gap-2" data-message-id="{{ $message->id }}">
                    ...
                    @else
                        <img src="{{ asset('images/user.png') }}" class="w-8 h-8 rounded-full object-cover"
                            alt="default avatar">
                    @endif
                    <div>
                        ...
                        <div
                            class="bg-gray-200 dark:bg-gray-800 text-gray-900 dark:text-gray-100 px-4 py-2 rounded-2xl rounded-bl-sm shadow-sm max-w-xs">
                            @if ($message->file_path)
                                @php
                                    $fileUrl = \Illuminate\Support\Facades\Storage::url($message->file_path);
                                    $fileExtension = strtolower(pathinfo($message->file_path, PATHINFO_EXTENSION));
                                ...
                                @else
                                    <a href="{{ $fileUrl }}" download
                                        class="flex items-center gap-2 text-gray-900 dark:text-gray-100 hover:underline mb-2">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none"
                                            viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
    ...
    <div class="border-t border-gray-300 dark:border-gray-700 bg-white dark:bg-transparent p-4">
        ...
        <form id="message-form" action="{{ route('room.message.store', $roomKey) }}" method="POST"
            ...
            <button type="button" id="select-file-btn"
                class="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700/50">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 stroke-[2.5]" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <path
                        d="M21.44 11.05l-8.49 8.49a5 5 0 0 1-7.07 0 5 5 0 0 1 0-7.07l8.49-8.49a3.5 3.5 0 0 1 4.95 4.95l-8.49 8.49a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l7.78-7.78" />
                </svg>
            </button>
            ...
            <button type="button" id="clear-file-btn"
                class="p-2 rounded-full hover:bg-gray-200 dark:hover:bg-gray-700/50" style="display:none;">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 stroke-[2.5]" fill="none" viewBox="0 0 24 24"
                    stroke="currentColor">
                    <line x1="18" y1="6" x2="6" y2="18" />
                    <line x1="6" y1="6" x2="18" y2="18" />
